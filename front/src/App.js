...
function App() {
  ...
  const [inputValue, setInputValue] = useState({ withdraw: "", deposit: "", bankName: "", transfer: "", address: "" });
  ...
  const contractAddress = '0xcA29799e974DceCc9ec9238a9346eBcF414A2936';
  ...
  const transferERC20Handler = async (event) => {
    console.log("transferERC20Handler", event);
    event.preventDefault();
    try {
      if (window.ethereum) {
        const provider = new ethers.providers.Web3Provider(window.ethereum);
        ...
        console.log(ethers.utils.parseEther(inputValue.transfer), inputValue.address);
        const txn = await bankContract.transferERC20(contractAddress, inputValue.address, ethers.utils.parseEther(inputValue.transfer));
        console.log(txn);

        console.log("Transfering money...");
        await txn.wait();
        console.log("Transferred money...done", txn.hash);

        customerBalanceHandler();

      } else {
        console.log("error to transfer");
        setError("Error to transfer.");
      }
    } catch (error) {
      console.log(error)
    }
  ...
  return (
    <main className="main-container">
      <h2 className="headline"><span className="headline-gradient">Flash Bank</span> 💰</h2>
